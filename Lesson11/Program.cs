...
////получаем имя потока
//Console.WriteLine($"Имя потока: {currentThread.Name}"); //за умовчанням властивість Name у об'єктів Thread не встановлено
//currentThread.Name = "Метод Main";
//Console.WriteLine($"Имя потока: {currentThread.Name}");
//Console.WriteLine($"В фоне ли поток: {currentThread.IsBackground}");
...
//// GetDomain        повертає посилання на домен програми
//// GetDomainID      повертає id домену програми, в якій виконується поточний потік
//// Sleep            зупиняє потік на певну кількість мілісекунд
//// Interrupt        перериває потік, який перебуває у стані WaitSleepJoin
//// Join             блокує виконання потоку, що викликав його доти, поки не завершиться потік, для якого був викликаний даний метод
...
//// Thread(ThreadStart) :                      як параметр приймає об'єкт делегата ThreadStart, який представляє виконувану в потоці дію
//// Thread(ThreadStart, Int32) :               на додаток до делегата ThreadStart приймає числове значення, яке встановлює розмір стека, що виділяється під  даний потік
//// Thread(ParameterizedThreadStart) :         як параметр приймає об'єкт делегата ParameterizedThreadStart, який представляє виконувану в потоці дію
//// Thread(ParameterizedThreadStart, Int32) :  разом із делегатом ParameterizedThreadStart приймає числове значення, яке встановлює розмір стека  для цього потоку
...
//// создаем новый поток:
//var myThread1 = new Thread(Print);
//var myThread2 = new Thread(new ThreadStart(Print));
//var myThread3 = new Thread(() => Console.WriteLine("Hello Threads"));
...
//var myThread = new Thread(Print);     // создаем новый поток
//myThread.Start();                     // запускаем поток myThread
////myThread.Join();                      // блокує виконання потоку, що викликав його доти, поки не завершиться потік
...
//void Print()                          // действия, выполняемые во втором потокке
//{
//    for (int i = 0; i < 10; i++)
//    {
...
////       myThread.Start();
//// Крім того, в головному потоці робимо аналогічні дії - виводимо на консоль числа від 0 до 4 із затримкою в 300 мілісекунд.
...
//// Подібно ми можемо створити і запускати і три, і чотири, і цілий набір нових потоків,які зможуть вирішувати ті чи інші завдання.
///////////////////////////////////////////////////////////////////////////////
...
//var myThread1 = new Thread(new ParameterizedThreadStart(Print));
//var myThread2 = new Thread(Print);
//var myThread3 = new Thread(message => Console.WriteLine(message));
...
//Thread myThread = new Thread(Print);    // создаем новый поток
//myThread.Start(number);                 // n * n = 16

//void Print(object? obj)                 // действия, выполняемые во втором потокке
//{
...
///////////////////////////////////////////////////////////////////////////////
//// у разі якщо нам треба додатково привести передане значення типу int, щоб його використовувати у обчисленнях.
//// якщо треба передати не один, а кілька параметрів різного типу? І тут можна визначити свої типи:
///////////////////////////////////////////////////////////////////////////////
...
//// в методі Main передаємо його у другий потік.
//// Але тут знову ж таки є одне обмеження: метод Thread.Star не є типобезпечним,
...
//// void Enter(object obj, bool acquiredLock) : додатково приймає другий параметр - логічне значення, яке вказує, чи отримано  володіння над об'єктом з першого параметра
...
//// PulseAll(object obj) : повідомляє всі потоки з черги очікування, що поточний потік звільнив об'єкт obj. Після чого один із потоків з черги очікування захоплює об'єкт obj.
//// bool TryEnter (object obj) : намагається захопити об'єкт obj. Якщо володіння над об'єктом успішно одержано, то повертається значення true
//// bool Wait (object obj) : звільняє блокування об'єкта та переводить потік у чергу очікування об'єкта. Наступний потік у черзі готовності об'єкта блокує цей об'єкт. А всі потоки, які викликали метод Wait, залишаються в черзі  очікування, доки не отримають сигналу від методу Monitor.Pulse або Monitor.PulseAll, надісланого власником блокування.
...
//// Set(); :      задає сигнальний стан об'єкта, дозволяючи одному або декільком потокам очікування продовжити роботу.
//// WaitOne() :   задає несигнальний стан та блокує поточний потік, доки поточний об'єкт AutoResetEvent не отримає сигнал.
...
//var waitHandler = new AutoResetEvent(true);  // объект-событие
...
//// Semaphore(int initialCount, int maximumCount) - параметр initialCountзадає початкову кількість потоків, а maximumCount- максимальну кількість потоків, які мають доступ до загальних ресурсів
//// Semaphore (int initialCount, int maximumCount, string? name) - на додаток задає ім'я семафора
//// Semaphore (int initialCount, int maximumCount, string? name, out bool createdNew) - останній параметр - createdNewпри значенні true вказує, що новий семафор був успішно створений. Якщо цей параметр дорівнює false, то семафор із зазначеним ім'ям вже існує
...
//class Reader
//{
//    static Semaphore sem = new Semaphore(3, 3/*, "SemName", out bool isCreated*/);     // создаем семафор
//    Thread myThread;
...
/////////////////////////////////////////////////////////////////////////////////
//// У програмі читач представлений класом Reader. Він інкапсулює всю функціональність, пов'язану з потоками через змінну Thread  myThread.
//// Сам семафор визначається у вигляді статичної змінної sem:
//// статичний семафор sem = новий семафор(3, 3);.
//// Його конструктор приймає два параметри:
//// перший - вказує, якій кількості об'єктів спочатку буде доступний семафор,
//// другий - вказує, яка максимальна кількість об'єктів буде використовувати цей семафор.
...
/////////////////////////////////////////////////////////////////////////////////
//// 1. Перший спосіб створення об'єкта Task та виклик у нього методу Start:
...
/////////////////////////////////////////////////////////////////////////////////
//// 2. Другий спосіб полягає у використанні статичного методу Task.Factory.StartNew() . 
...
/////////////////////////////////////////////////////////////////////////////////
//// 3. Третій спосіб визначення та запуску задач представляє використання статичного методу Task.Run() :
...
//Task task1 = new Task(() =>             // создаем Task
//{
...
//Console.WriteLine("Main Starts");
//var task1 = new Task(() =>              // создаем задачу
//{
//    Console.WriteLine("Task Starts");
//    Thread.Sleep(1000);                 // задержка на 1 секунду - имитация долгой работы
//    Console.WriteLine("Task Ends");
...
///////////////////////////////////////////////////////////////////////////////////////
//// * Синхронний запуск Task:
///////////////////////////////////////////////////////////////////////////////////////
//// За замовчуванням Task запускаються асинхронно. Однак за допомогою методу RunSynchronously() можна запускати синхронно:
...
//// int result = sumTask.Result;
//// При цьому при зверненні до властивості Result поточний потік зупиняє виконання і чекає, коли буде отримано результат виконуваного завдання.
///////////////////////////////////////////////////////////////////////////////////////
//// Інший приклад:
...
///////////////////////////////////////////////////////////////////////////////////////
//// Завдання продовження або continution task дозволяють визначити завдання, які виконуються після завершення інших завдань.
//// Завдяки цьому ми можемо викликати після виконання одного завдання кілька інших, визначити умови їхнього виклику, передати з
//// попереднього завдання до наступного деякі дані.
...
//void Square(int n)  // вычисляем квадрат числа
//{
//    Console.WriteLine($"Выполняется задача {Task.CurrentId}");
//    Thread.Sleep(3000);
